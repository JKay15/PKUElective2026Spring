...
class CompetitionErrorRotationOfflineTest(unittest.TestCase):
    def test_error_rotation_then_success(self):
        orig = {
        ...
        def _draw(self, **kwargs):
            draw_calls["n"] += 1
            if draw_calls["n"] == 3:
                raise OperationTimeoutError(msg="timeout")
            return _DummyResp()
        ...
        try:
            with mock.patch.object(loop.config.__class__, "courses", new=property(lambda self: courses)), \
                ...
                self.assertGreaterEqual(elect_calls["n"], 3)
