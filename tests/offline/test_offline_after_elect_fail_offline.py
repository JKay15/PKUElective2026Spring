...
from autoelective.exceptions import ElectionFailedError
...
class OfflineAfterElectFailOfflineTest(unittest.TestCase):
    def test_captcha_success_then_offline(self):
        orig = {
        ...
        loop.OFFLINE_OBSERVE_SECONDS = 0.5
        loop.OFFLINE_OBSERVE_MIN_REFRESH = 0.2
        ...
        def _supply_cancel(self, username, **kwargs):
            ...
            if supply_calls["n"] <= 1:
                return _DummyResp()
            if supply_calls["n"] <= 3:
                raise RequestException("simulated network fail")
            return _DummyResp()
        ...
        try:
            with mock.patch.object(loop.config.__class__, "courses", new=property(lambda self: courses)), \
                ...
                start = time.time()
                while loop.environ.runtime_stats.get("offline_recover", 0) < 1 and time.time() - start < 5.0:
                    time.sleep(0.05)
