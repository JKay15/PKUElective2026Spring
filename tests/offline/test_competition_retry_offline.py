...
from autoelective.exceptions import QuotaLimitedError, ElectionSuccess
...
class CompetitionRetryOfflineTest(unittest.TestCase):
    def test_retry_after_quota_limited_then_success(self):
        orig = {
        ...
        try:
            with mock.patch.object(loop.config.__class__, "courses", new=property(lambda self: courses)), \
                ...
                self.assertGreaterEqual(elect_calls["n"], 2)
